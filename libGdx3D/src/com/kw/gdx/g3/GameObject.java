...
import com.kw.gdx.g3.math.Circle;
import com.kw.gdx.g3.math.GdxMath;
...
public class GameObject extends ModelBase{
    ...
    public void act(float delta) {
        ...
        moveTo.set(tempV2.x,tempV2.y,-60);
        moveTo(moveTo);
    }

    private Vector3 roV3 = new Vector3();

    private Vector3 scaleV3 = new Vector3();
    private Quaternion q = new Quaternion();

    public static float scaleModel = 1120;
    @Override
    public void draw(Batch batch, float parentAlpha) {
        batch.getTransformMatrix().getScale(scaleV3);

        for (Node node : modelInstance.nodes) {
            node.scale.set(3*scaleModel*scaleV3.x,6*scaleModel*scaleV3.y,1*scaleModel*scaleV3.x);
        }
        modelInstance.calculateTransforms();
